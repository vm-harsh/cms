...
export default function FacultyEditCoursePage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setError(err.message || 'Failed to load course details or access denied');
      } finally {
        setLoading(false);
      }
    }
  ...
  if (error) {
    return <ErrorState message={error} onRetry={() => navigate('/faculty/courses')} />;
  }
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">
            Update course syllabus, duration, and curriculum description
          </p>
        </div>
      </div>
